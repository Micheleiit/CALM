// QUESTO CODICE MANDA I DATI DELLA POSIIZIONE DEL MOUSE AL RM57: CAPIRE COME VIENE CODIFICATA ESATTAMENTE LA POSIZIONE!!!! 
...
void mouseMoved() {  
  ...
  //int xChange = mouse.getXChange();  // Riporta la quantità relativa di movimento del mouse sull'asse X dall'ultimo polling.
  //roll = xChange;
  roll = mouse.getXChange();
  //int yChange = mouse.getYChange();  // Riporta la quantità relativa di movimento del mouse sull'asse Y dall'ultimo polling.
  //pitch = yChange;
  pitch = mouse.getYChange();

  // Invia i dati solo se c'è movimento
  if (roll != 0 || pitch != 0) {
    ...
    /* Accendi i LED in base al movimento
    if (xChange != 0) {
