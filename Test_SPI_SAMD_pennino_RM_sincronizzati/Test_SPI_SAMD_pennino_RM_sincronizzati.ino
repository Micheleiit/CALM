...
void loop() {
  ...
  if (!readyToSend) {  // Controlla se è pronto per inviare i dati del mouse

    delay(100); // opzionale per fare il polling periodico in ms

    readyToSend = checkSPIFlag();
      ...
      DEBUG_PRINT("Pitch: ");
      DEBUG_PRINT(pitch_mm);  DEBUG_PRINT("\n");
    ...
    if(readyToSend){ // sono pronto a inviare i dati del mouse
      digitalWrite(LED_OK, HIGH);
      digitalWrite(LED_FAULT, HIGH);
      digitalWrite(BUZZER, HIGH);
      delay(500);
...
bool checkSPIFlag() {
  ...
  // Faccio Polling al RM57 mandandogli un dato di Roll = 0 e un dato di Pitch = 0 (Invia un comando vuoto per leggere il valore mandato da RM57)
  prepareData(roll_mm, pitch_mm, tx); // mando i dati in mm di spostamento

  SPI1.beginTransaction(settings_spi);    // Leave the SPI Data Formats as is on RM57
  digitalWrite(IPC_SPI_CS, LOW);          // Abbassa il Chip Select (CS) per attivare la comunicazione SPI.
  rx[0] = SPI1.transfer16(tx[0]);         // Transfer block 1: ROLL LSB 
  delayMicroseconds(1);
  digitalWrite(IPC_SPI_CS, LOW);
  rx[1] = SPI1.transfer16(tx[1]);         // Transfer block 2: ROLL MSB
  digitalWrite(IPC_SPI_CS, HIGH);
  delayMicroseconds(1);
  digitalWrite(IPC_SPI_CS, LOW);
  rx[2] = SPI1.transfer16(tx[2]);         // Transfer block 3: PITCH LSB
  digitalWrite(IPC_SPI_CS, HIGH);
  delayMicroseconds(1);
  digitalWrite(IPC_SPI_CS, LOW);
  rx[3] = SPI1.transfer16(tx[3]);         // Transfer block 4: PITCH MSB
  digitalWrite(IPC_SPI_CS, HIGH);
  SPI1.endTransaction();
  ...
  //return (receivedValue == TRIGGER_VALUE);
  return (rollRX >= TRIGGER_VALUE);
}
