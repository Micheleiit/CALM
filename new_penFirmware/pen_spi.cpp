...
void initSPIStruct(SPIStruct* spiStruct){
  ...
  spiStruct->roll_mm = 0;
  spiStruct->pitch_mm = 0;
  
  //
  spiStruct->rollBuffer[BUFFER_SIZE] = {0};
  spiStruct->pitchBuffer[BUFFER_SIZE] = {0};  
...
void handlePenMotionAndSendSPI(SPIStruct* spiStruct){
  ...
  switch(current_state){
    ...
    case FREE_HAND:
      ...
      // accumulo gli spostamneti dx e dy catturati dalle chiamate di getXChange() e getYChange()
    ...
    case DRAW_RECORD:
      ...
      abort_sequence = !read_trajectory_cst_speed(traj_record, &spiStruct->rollTX, &spiStruct->pitchTX, 50000, 0);
  ...
  // converto gli spostamenti in micrometri (mm)
  spiStruct->roll_mm = spiStruct->rollTX*1000/30;
  spiStruct->pitch_mm = spiStruct->pitchTX*1000/30;

  int32_t original_roll_mm = spiStruct->roll_mm ;
  int32_t original_pitch_mm = spiStruct->pitch_mm;
  ...
  prepareSPIData(spiStruct->roll_mm, spiStruct->pitch_mm, tx);
...
void mouseMoved() {
  ...
  int32_t xChange = usbManager.mouse.getXChange(); // restituisce la variazione dell'asse X del mouse rispetto alla sua posizione precedente
  int32_t yChange = usbManager.mouse.getYChange(); // restituisce la variazione dell'asse Y del mouse rispetto alla sua posizione precedente

  //Aggiorna le variabili delta_roll e delta_pitch
  penSpi.delta_roll = ((-xChange) * SCALING_MOUSE);
  penSpi.delta_pitch = ((-yChange) * SCALING_MOUSE);
  ...
  switch(current_state){
    ...
    case RECORDING:
      ...
      for(int i = 0; i < sizeof(tx) / sizeof(tx[0]); i++){ // Itera sul numero di elementi del vettore tx
        ...
        digitalWrite(spiManager->csPin, LOW);     // Seleziona lo slave
        rx[i] = SPI1.transfer16(tx[i]);           // Transfer block 1:  Trasmette un blocco di 16 bit e riceve un blocco di 16 bit simultaneamente
        digitalWrite(spiManager->csPin, HIGH);    // Deseleziona lo slave
        delayMicroseconds(1);
      }
...
void deleteOutliers(SPIStruct* spiStruct){
  ...
  spiStruct->rollBuffer[spiStruct->bufferIndex] = spiStruct->roll_mm;
  spiStruct->pitchBuffer[spiStruct->bufferIndex] = spiStruct->pitch_mm;
  ...
  if(spiStruct->sampleCount == BUFFER_SIZE){
    ...
    spiStruct->roll_mm = rollSum / BUFFER_SIZE;
    spiStruct->pitch_mm = pitchSum / BUFFER_SIZE;
