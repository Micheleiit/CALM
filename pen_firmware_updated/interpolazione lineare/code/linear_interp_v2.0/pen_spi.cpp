...
void handlePenMotionAndSendSPI(SPIStruct* spiStruct){
  ...
  switch(current_state){
    ...
    case DRAW_RECORD:
      ...
      abort_sequence = !read_trajectory_cst_speed(traj_record, &spiStruct->rollTX, &spiStruct->pitchTX, INTERP_STEP, 0);
...
void mouseMoved() {
  ...
  penSpi.delta_roll = ((-xChange_micro) * SCALING_MOUSE);
  penSpi.delta_pitch = ((-yChange_micro) * SCALING_MOUSE);
