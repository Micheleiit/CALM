...
void setup() {
  ...
  //myapp = initAppStruct(&spiManager, &usbManager, &penSpi, &penUsb, &penMotion, &zoom, &rightButton, &leftButton, &middleButton, &ledOk, &ledFault, &ledOnOff, &ledRed, &ledGreen);
  /*********************************************************************************************************************************************************************************/
...
void loop() {
  ...
  if(current_state != ERROR){ // Manipolatore pronto a processare gli input del pennino
    ...
    if(current_state == FREE_HAND || current_state == RECORDING){
      ...
      /* In this case, the position is send at 1kHz ovvero dopo 1ms.*/
...
if(myapp->penSpi->slaveReady && myapp->penUsb->USBready){ // slave pronto a processare gli input del pennino
  ...
  if(myapp->penMotion->restart){
    //digitalWrite(BUZZER, HIGH); // DEBUG VISIVO
